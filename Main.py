...
# 1) Page setup
st.set_page_config(
    page_title="Agency Policy Generator",
    layout="wide",
    page_icon="🔍",
)
...
def main():
    ...
    with col2:
        st.header("Agency Policy Generator")
        st.write("Drag and drop your PDF or Excel file here")
    umbrella_data = None
    auto_forms_sections = {}
    ...
    if not uploaded_files:
        ...
        wc_pdf_bytes = None
        excel_file = None
        pdf_files = [file for file in uploaded_files if file.name.split('.')[-1].lower() == 'pdf']
        for file in uploaded_files:
            if file.name.split('.')[-1].lower() in ['xlsx', 'xls'] and excel_file is None:
                excel_file = file
        if len(pdf_files) == 1:
            file = pdf_files[0]
            if ("wc" in file.name.lower() or "workers comp" in file.name.lower() or "workers compensation" in file.name.lower()):
                wc_pdf_bytes = file.read()
                st.write(f"Detected Workers Compensation PDF (only one file): {file.name}")
            else:
                main_pdf_bytes = file.read()
                st.write(f"Detected main policy PDF (only one file): {file.name}")
        else:
            for file in pdf_files:
                if ("wc" in file.name.lower() or "workers comp" in file.name.lower() or "workers compensation" in file.name.lower()):
                    wc_pdf_bytes = file.read()
                    st.write(f"Detected Workers Compensation PDF: {file.name}")
                else:
                    main_pdf_bytes = file.read()
                    st.write(f"Detected main policy PDF: {file.name}")
    ...
    if processing_main:
        ...
        st.write("DEBUG: Auto Loss Payees extraction output", df_loss_payees)
    ...
    if processing_main and InlandMarine is not None:
        ...
        if excel_file is not None:
            ...
            st.sidebar.markdown("### Inland Marine Excel Debug")
            st.sidebar.text(excel_debug)
    ...
    if processing_main:
        ...
        placeholders = {
            ...
            "<Named Insured>": policy_info.get("Named Insured", ""),
            "<Quote No.>": chosen_no,
            "<Policy No.>": chosen_no,
            "<Policy Period>": policy_info.get("Proposed Policy Period", ""),
            "<Agent Name>": policy_info.get("Agent Name", ""),
        ...
        try:
            ...
            wc_policy_info_dict = {}
        chosen_no = wc_policy_info_dict.get("Quote No.", wc_policy_info_dict.get("Policy No.", ""))
        placeholders = {
            "<Insured Name>": wc_policy_info_dict.get("Named Insured", "N/A"),
            "<Named Insured>": wc_policy_info_dict.get("Named Insured", "N/A"),
            "<Quote No.>": chosen_no,
            "<Policy No.>": chosen_no,
            "<Policy Period>": wc_policy_info_dict.get("Proposed Policy Period", ""),
            "<Agent Name>": wc_policy_info_dict.get("Agent Name", ""),
            "<Date>": datetime.datetime.today().strftime("%m/%d/%Y"),
            "<Terrorism Premium>": "",
            "<Rating Company>": wc_policy_info_dict.get("Rating Company", ""),
        }
    ...
    # ---------------------------
    # CONDITIONAL TEXAS FORM INSERTION (PDF version)
    # ---------------------------
    texas_vehicle_found = False
    if processing_main and not df_auto3.empty and "State" in df_auto3.columns:
        texas_vehicle_found = df_auto3["State"].astype(str).str.strip().str.upper().eq("TX").any()
    
    # ---------------------------
    # DOWNLOAD BUTTON
    # ---------------------------
    word_io = BytesIO()
    # Final pass to ensure all new placeholders in appended content are replaced
    replace_placeholders_selectively(word_doc, placeholders)
